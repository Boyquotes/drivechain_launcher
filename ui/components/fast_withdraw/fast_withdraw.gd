...
func _on_button_invoice_paid_pressed() -> void:
	...
	var chain_name = $ChainSelectionButton.get_item_text($ChainSelectionButton.get_selected())
	$"/root/Net".invoice_paid.rpc_id(1, chain_name, txid, $SpinBoxAmount.value, $LineEditMainchainAddress.text)


func _on_button_request_invoice_pressed() -> void:
	# Send fast withdraw request only to server
	var chain_name = $ChainSelectionButton.get_item_text($ChainSelectionButton.get_selected())
	$"/root/Net".request_fast_withdraw.rpc_id(1, chain_name, $SpinBoxAmount.value, $LineEditMainchainAddress.text)
